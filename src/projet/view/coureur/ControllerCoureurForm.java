...
public class ControllerCoureurForm {
	...
	private void initialize() {
		...
		brSexe1.selectedToggleProperty().addListener(l -> {
			if (brSexe1.getSelectedToggle().equals(rbHomme1))
				courant1.setSexe("Homme");
			if (brSexe1.getSelectedToggle().equals(rbFemme1))
				courant1.setSexe("Femme");
		});
		...
		// champ complex
		UtilFX.bindBidirectional(dpNaissance2.getEditor(), courant2.naissanceProperty(),
				new ConverterStringLocalDate());
		brSexe2.selectedToggleProperty().addListener(l -> {
			if (brSexe2.getSelectedToggle().equals(rbHomme2))
				courant2.setSexe("Homme");
			if (brSexe2.getSelectedToggle().equals(rbFemme2))
				courant2.setSexe("Femme");
		});
	}
